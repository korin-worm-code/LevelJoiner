...
class WormDBStufFactory(object):
	...
	def __init__(self,basename):
		self.layer_name = basename
		self.points_name = basename + '_points'
		self.levels_name = basename + '_levels'
		self.levels_points_name = basename + '_levels_points'
	
	class WormLevel(Base,WormLevelBase):
		__tablename__ = self.levels_name
		point = relationship('WormPoint', secondary=self.levels_points_name)
		
	class WormPoint(Base,WormPointBase):
		__tablename__ = self.points_name
		level = relationship('WormLevel', secondary=self.levels_points_name)
		
	class WormLevelPoints(Base,WormLevelPointsBase):
		__tablename__ = self.levels_points_name
		# This table has a "composite primary key" composed of the first 2 ForeignKey entries and the internal primary key
		# This is the level_id in the external table
		worm_level_id = Column(Integer, ForeignKey(self.levels_name + '.worm_level_id'), primary_key=True)
		# This is the point id of the END point of a line segment.
		point_id = Column(Integer, ForeignKey(self.points_name + '.worm_point_id'), primary_key=True)
		# Database magic that links entries in this table with entries in another table
		worm_level = relationship(WormLevel, backref=backref("worm_point_assoc"))
		# Database magic that links entries in this table with entries in another table
		worm_point = relationship(WormPoint, backref=backref("worm_level_assoc"))
